...
def mod_ts(line, inFmt, outFmt, printall, verbose, ignore):
	...
	for fmt in inFmt:
		...
		for i, candidate in present_fmt.items():
			...
			# One match, this is what we want!
			elif len(match) == 1:
	...
	if method == 'enquire':
		...
		for file in args.infile:
			for line in load(file):
				...
				new_line = mod_ts(line, args.informat, args.outformat, printall=args.printall, ignore=args.ignore, verbose=args.verbose)
				if new_line:
